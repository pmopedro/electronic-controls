String input;
char terminator = ';';

unsigned char v[] = {};

unsigned char v01[] = {};

unsigned char v02[] = {};

unsigned char v03[] = {};

unsigned char v04[] = {};

unsigned char v05[] = {};

unsigned char v06[] = {};

unsigned char v07[] = {};

unsigned char v08[] = {};

unsigned char v09[] = {};

unsigned char v10[] = {};

unsigned char v11[] = {};

unsigned char v12[] = {};


void copy(unsigned char* src, unsigned char* dst, int len) {
    memcpy(dst, src, sizeof(src[0])*len);
}

void setup(){
    Serial.begin(9600);
     // Função para realizar o setup do arduino (roda uma única vez)
     
     DDRD = B11111111; // Condifura todo registrador da porta D como saída. 
     // Equivale a fazer pinMode(porta, OUTPUT) para porta = 1 a 8. 
     Serial.begin(9600);
     PORTD = B00000000; // Seta todo registrador D como saúda (OUTPUT)       // Delay inicial de 3s
}

void loop(){

    int tam = sizeof(v) / sizeof(int); // Obtém o tamanho do vetor
    PORTD = B00000000; // Seta todo registrador D como saúda (OUTPUT)
    delay(3000);       // Delay inicial de 3s
    
    if(Serial.available()){
        input = Serial.readStringUntil(terminator);
        Serial.print("Você entrou com o vetor:");
        Serial.println(input);
    }
    if(input.equals("v01")) {
         for (int i=0; i < tam; i++){
           PORTD = v01[i]; // Fornece o valor desejado no registrador PORTD, 
           Serial.print(PORTD,HEX);
           Serial.print('\n');
           //de forma sequencial, a partir do incremente de i
           delay(500); // Delay de 1s
        } 
     };
    if(input.equals("v02")) {
         for (int i=0; i < tam; i++){
           PORTD = v02[i]; // Fornece o valor desejado no registrador PORTD, 
           Serial.print(PORTD,HEX);
           Serial.print('\n');
           //de forma sequencial, a partir do incremente de i
           delay(500); // Delay de 1s
        } 
     };
    if(input.equals("v03")){
         for (int i=0; i < tam; i++){
           PORTD = v03[i]; // Fornece o valor desejado no registrador PORTD, 
           Serial.print(PORTD,HEX);
           Serial.print('\n');
           //de forma sequencial, a partir do incremente de i
           delay(500); // Delay de 1s
        } 
     };
    if(input.equals("v04")) {
         for (int i=0; i < tam; i++){
           PORTD = v04[i]; // Fornece o valor desejado no registrador PORTD, 
           Serial.print(PORTD,HEX);
           Serial.print('\n');
           //de forma sequencial, a partir do incremente de i
           delay(500); // Delay de 1s
        } 
     };
    if(input.equals("v05")) {
         for (int i=0; i < tam; i++){
           PORTD = v05[i]; // Fornece o valor desejado no registrador PORTD, 
           Serial.print(PORTD,HEX);
           Serial.print('\n');
           //de forma sequencial, a partir do incremente de i
           delay(500); // Delay de 1s
        } 
     };
    if(input.equals("v06")) {
         for (int i=0; i < tam; i++){
           PORTD = v06[i]; // Fornece o valor desejado no registrador PORTD, 
           Serial.print(PORTD,HEX);
           Serial.print('\n');
           //de forma sequencial, a partir do incremente de i
           delay(500); // Delay de 1s
        } 
     };
    if(input.equals("v07")) {
         for (int i=0; i < tam; i++){
           PORTD = v07[i]; // Fornece o valor desejado no registrador PORTD, 
           Serial.print(PORTD,HEX);
           Serial.print('\n');
           //de forma sequencial, a partir do incremente de i
           delay(500); // Delay de 1s
        } 
     };
    if(input.equals("v08")) {
         for (int i=0; i < tam; i++){
           PORTD = v08[i]; // Fornece o valor desejado no registrador PORTD, 
           Serial.print(PORTD,HEX);
           Serial.print('\n');
           //de forma sequencial, a partir do incremente de i
           delay(500); // Delay de 1s
        } 
     };
    if(input.equals("v09")) {
         for (int i=0; i < tam; i++){
           PORTD = v09[i]; // Fornece o valor desejado no registrador PORTD, 
           Serial.print(PORTD,HEX);
           Serial.print('\n');
           //de forma sequencial, a partir do incremente de i
           delay(500); // Delay de 1s
        } 
     };
    if(input.equals("v10")){
         for (int i=0; i < tam; i++){
           PORTD = v10[i]; // Fornece o valor desejado no registrador PORTD, 
           Serial.print(PORTD,HEX);
           Serial.print('\n');
           //de forma sequencial, a partir do incremente de i
           delay(500); // Delay de 1s
        } 
     };
    if(input.equals("v11")){
         for (int i=0; i < tam; i++){
           PORTD = v11[i]; // Fornece o valor desejado no registrador PORTD, 
           Serial.print(PORTD,HEX);
           Serial.print('\n');
           //de forma sequencial, a partir do incremente de i
           delay(500); // Delay de 1s
        } 
     };
    if(input.equals("v12")){
         for (int i=0; i < tam; i++){
           PORTD = v12[i]; // Fornece o valor desejado no registrador PORTD, 
           Serial.print(PORTD,HEX);
           Serial.print('\n');
           //de forma sequencial, a partir do incremente de i
           delay(500); // Delay de 1s
        } 
     };

    

    // repetição para percorrer todo vetor
    Serial.print("Entre com o próximo vetor:");
    Serial.print('\n');
    
}